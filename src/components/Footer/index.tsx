...
interface FeatureLinkGroupProps {
  features: string[];
}

const FeatureLink: FC<FeatureLinkGroupProps> = ({ features }) => (
...
const Footer: FC = () => {
  const featureLinks = [['Features', 'Wallet'], ['FAQ', 'Contacts'], ['Terms & Conditions']];

  return (
    <footer className="flex items-center justify-center self-stretch bg-black px-16 py-20 max-md:px-5">
      <div className="flex w-[1136px] max-w-full flex-col">
        ...
        <section className="mt-16 flex flex-wrap content-center gap-5 self-start whitespace-nowrap py-1 text-base leading-6 text-white max-md:mt-10">
          {featureLinks.map((group, index) => (
            <FeatureLink key={index} features={group} />
          ))}
          <div className="grow self-start">Swap</div>
          <div className="self-start">OTC</div>
        </section>
        <section className="mt-16 text-xs leading-5 text-white max-md:mt-10 max-md:max-w-full">
          2023 © All rights reserved
          <p>
            Piastra Pay – a limited liability company registred in Poland with registration number 520307010 and a
            license in the Register of Payment Services of Poland under number MIP 155/2022
          </p>
        </section>
      </div>
    </footer>
